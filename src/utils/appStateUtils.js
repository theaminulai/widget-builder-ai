...
export const mapCanonicalToNamedFiles = (files, title) => {
	...
	const phpContent =
		input[names.php] ??
		input['widget.php'] ??
		Object.entries(input).find(([key]) => key.endsWith('.php'))?.[1] ??
		'';
	const cssContent =
		input[names.css] ??
		input['style.css'] ??
		Object.entries(input).find(([key]) => key.endsWith('.css'))?.[1] ??
		'';
	const jsContent =
		input[names.js] ??
		input['script.js'] ??
		Object.entries(input).find(([key]) => key.endsWith('.js'))?.[1] ??
		'';
	const mapped = {
...
export const normalizeChatMessage = (message, index = 0) => {
	...
	return {
		...
		id:
			safeMessage.id ||
			`msg-${Date.now()}-${index}-${Math.random().toString(36).slice(2, 8)}`,
